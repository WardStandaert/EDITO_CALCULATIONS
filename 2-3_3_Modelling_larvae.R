...
for (s in 1:nrow(species_sp)) {
  ...
  tmp_full_dat <- left_join(presences_list_sp[[which(names(presences_list) == species_sp$simple[s])]] %>% 
                              mutate(lon = Longitude, lat = Latitude), full_dat_sp, by = c("lon","lat")) %>%
    filter(scientificname == species_sp$scientific[s])
  
  red_dat_sp[[s]] <- tmp_full_dat
  print(name)
}

reduced_dat <- bind_rows(red_dat_sp)
...
# eval_res_list_sp <- eval_res_list
...
#After sp & geo filtering:  Moran's I value of 0.06, expected -0.00007; significant
morans_I_list[[2]]   
#Before filtering:          Moran's I value of , expected ; significant
#After sp & geo filtering:  Moran's I value of 0.01, expected -0.00005; significant
...
#After sp filtering:        Moran's I value of 0.17, expected -0.0002; significant
...
for (s in 1:nrow(species_sp)) {
  ...
  for (v in 1:length(which(v_imp > 0.08))) {
...
df_evol <- df_evol %>%
  ...
  filter(month %in% c("Sep", "Oct", "Dec", "Jan"))
...
ggsave(paste0("evolution/larvae_monthly_evolution_HSI_average.png"), width = 10, height = 4)
...
ggsave(paste0("evolution/larvae_yearly_evolution_HSI_average.png"), width = 10, height = 4)
...
ggplot(df, aes(x = lat, y = val, group = year)) +
  ...
  scale_x_continuous(limits = c(47.5,60)) +
