...
datasetID = "cmems_obs-oc_glo_bgc-transp_my_l4-gapfree-multi-4km_P1D";  # datasetID
...
head(cbind(lon, lat), n = 20) ##lon goes upward, lat goes down
...
offset <- c(lon_indices[1], lat_indices[1])    #lon,lat
count <- c(lon_range, lat_range)               #dimension var
...
secchi_array <- array(c(`values-2000-1`, `values-2000-2`, `values-2000-3`, `values-2000-4`, `values-2000-5`, `values-2000-6`, 
...
var_secchi <- ncvar_def("ZSD", "m", list(dim_lon, dim_lat, dim_time), -999, longname = "Secchi disk depth - Mean of the binned pixels", prec = "float")
...
ncnew_secchi <- nc_create("secchi_depth.nc", var_secchi)
ncvar_put(ncnew_secchi, var_secchi, secchi_array, start = NA, count = NA)
nc_close(ncnew_secchi)
