...
load("~/INPUT/bio_dat_co_spawning.Rdata")
rm(reduced_dat, full_dat)

setwd("~/OUTPUT/reduced_data_model/spawning/final_model")
...
# Variables selected per species_sp
# Twaite shad includes all vars except for chlorophyll because the variable is correlated with seabed_energy

v_list <- list()
v_list$herring <- c("SST", "SSS", "windfarms", "seabed_energy", "seabed_substrate", "depth")
v_list$mackerel <- c("SST", "SSS", "windfarms", "max_SSV", "depth")
...
substr_lvl$sub_char <- as.character(substr_lvl$sub_char)

energy_lvl <- data.frame(ene_char = c("High energy", "Moderate energy", "Low energy", "No energy information"),
                         seabed_energy = c(1:4))
energy_lvl$ene_char  <- as.character(energy_lvl$ene_char)
...
lapply(pr_pa, head, n=2)              #pr points presence/absence values per species_sp
lapply(pr_coord, head, n=2)           #pr points coordinates per species_sp
lapply(pr_predv, head, n=2)           #pr points covariate extracts per species_sp
...
for (s in 1:nrow(species_sp)) {
  ...
  bg_predv[[s]] <- tmp_bckgr %>%  #make seabed habitat a factor
    dplyr::select(all_of(v_list[[which(names(v_list) == species_sp$simple[s])]])) #variable selection
...
#     mutate(Month = as.factor(Month)) %>%  #make seabed habitat a factor
#     select(all_of(v_list[[which(names(v_list) == species_sp$simple[s])]])) #variable selection
# 
#   names(bg_pa)[s] <- names(bg_coord)[s] <- names(bg_predv)[s] <- species_sp$simple[s]
# }
# 
# lapply(bg_pa, head, n=2)              #bg points presence/absence values per species_sp
# lapply(bg_coord, head, n=2)           #bg points coordinates per species_sp
# lapply(bg_predv, head, n=2)           #bg points covariate extracts per species_sp
# 
...
#      file = "SAVE/BPNS_presence_background.Rdata")

load("SAVE/BPNS_presence_background.Rdata")
...
for (s in 1:nrow(species_sp)) {
  ...
  print(paste0(species_sp$simple[s], " done"))
}

# save(eval_res_list, file = "SAVE/BPNS_ENMeval_outcomes.Rdata")

# SAVE ----
load("SAVE/BPNS_ENMeval_outcomes.Rdata")

for(s in 1:nrow(species_sp)) print(eval_res_list[[s]] %>%  eval.results() %>% filter(delta.AICc == 0))
...
#     
#   names(model_list)[[s]] <- species_sp$simple[s]
#   print(paste("finished:", species_sp$simple[s]))
# }
...
#   names(model_list_enm)[[s]] <- species_sp$simple[s]
#   print(paste("finished:", species_sp$simple[s]))
# }
# 
...
# 
# prediction1 <- predict(plot_st , mod.seq, type = "cloglog", 
...
for (s in 1:nrow(species_sp)) {
  ...
  for (i in 1:10) {
    ...
    maxent_model <- dismo::maxent(x = rbind(TrainPres, TrainBg),
  ...
  names(AUC_maxent)[s] <- names(TSS_maxent)[s] <- species_sp$simple[s]
  toc()
}
...
for (s in 1:nrow(species_sp)) {
  ...
  colnames(AUC_maxent_vect)[s] <- colnames(TPR_maxent_vect)[s] <- 
    colnames(TNR_maxent_vect)[s] <-  species_sp$simple[s]
}
...
plot(factor(AUC_maxent_df$variable, 
            levels = c("herring", "mackerel"), 
            labels = c("Atlantic herring", "Atlantic mackerel")), 
...
sum_all <- cbind(sum_auc, sum_tss)
sum_all

setwd("/data/home/innovauth/ward.standaert@vliz.be/OUTPUT/reduced_data_model/spawning/final_model/evaluation/")

write.csv(sum_all, "BPNS_validation_metrics.csv")
...
## boxplots ####
png("BPNS_AUC_boxplot.png", width = 6, height =6, res = 100, units = 'in')
...
png("BPNS_TSS_boxplot.png", width = 6, height =6, res = 100, units = 'in')
par(las = 2, mar = c(6, 5, 2, 2), mgp = c(3, 0.5, 0))
boxplot(
...
morans_I_list[[1]]   
#Before filtering:          Moran's I value of , expected : significant
#After sp & geo filtering:  Moran's I value of 0.08, expected -0.00007; significant
morans_I_list[[2]]   
#Before filtering:          Moran's I value of , expected ; significant
#After sp & geo filtering:  Moran's I value of 0.03, expected -0.00005; significant
...
morans_I_list[[2]]   
#Before filtering:          Moran's I value of , expected -; significant
#todo After sp & geo filtering:  Moran's I value of 0.07, expected -0.01; significant


# setwd("/data/home/innovauth/ward.standaert@vliz.be/full_dat_sp_outcomes")
# save(model_list, 
#      pr_predv, bg_predv, 
#      pr_pa, bg_pa, 
#      pr_coord, bg_coord, file = "full_dat_sp_modelling_output.Rdata")
...
# 
# 
# x_full <- full_dat_sp %>% filter(scientificname == species_sp$scientific[s])
# x_red <- reduced_dat_sp %>% filter(scientificname == species_sp$scientific[s])
# 
# tab_fun <- function(l) {
...
#     b <- tab_fun(cor_x_red)
#     
#     j <- left_join(a, b %>% select(id, statistic, p_value), by = "id")
#     tmp_list[[st]] <- j
#   }
#   SAC_list[[s]] <- tmp_list
#   print(species_sp$simple[s])
# }
...
# save(tib_all_v, file = "tib_all_v.rData")
...
save(cor_maxnet, file = "SAVE/BPNS_variable_importance.Rdata")
...
for (s in 1:nrow(species_sp)) {
  png(paste0("BPNS_",species_sp$simple[s], "_var_imp_response_curves.png"), width = 13, 
  ...
  for (v in 1:length(which(v_imp > 0.02))) {
...
#8. predictions BPNS ----
#convert layers with different units first!

rename_key <- data.frame(from = c("dissolved_oxygen","euphotic_depth","mSSV","phytoplankton",
                                  "sea_surface_salinity","sea_surface_temperature","zooplankton","seabed_habitat"),
                         to = c("DO","EuphD","max_SSV","Phyto","SSS","SST","ZooPl","seabed_substrate"))
#substrate needs conversion since it was coded differently for BPNS:
substr_key_bpns <- data.frame(levels = c("Coarse substrate", "Sand", "Muddy sand", "Sandy mud", "Seabed", "Mixed sediment", "Fine mud or Sandy mud or Muddy sand"),
                              labels = c(1:7))
...
for (s in 1:nrow(species_sp)) {
  ...
  for (m in 1:12) {
    ...
    names(plot_st) <- 
      ifelse(names(plot_st) %in% rename_key$from, 
             as.character(rename_key$to[match(names(plot_st), rename_key$from)]), 
             names(plot_st))
    ...
    values(plot_st[[which(str_detect(names(plot_st),"ZooPl"))]]) <- values(plot_st[[which(str_detect(names(plot_st),"ZooPl"))]])*2.5*0.5 #depth of 0.5m
    
    
    pr_geo_bpns <- stack(pr_geo_bpns, predict(plot_st, mod, clamp=T, type="cloglog", 
                                              factors = list(ene_char = factor(energy_lvl$seabed_energy,
                                                                               labels = energy_lvl$ene_char, 
                                                                               levels = energy_lvl$seabed_energy),
                                                             sub_char = factor(substr_key_bpns$labels,
                                                                               labels = substr_key_bpns$levels, 
                                                                               levels = substr_key_bpns$labels))))
    names(pr_geo_bpns)[m] <- month.abb[m]
  }
  
  is_not_na <- !is.na(pr_geo_bpns[[1]])
  min(is_not_na)
  
  mask_layer <- pr_geo_bpns[[1]]
  values(mask_layer) <- 1
  values(mask_layer)[which(is.na(values(pr_geo_bpns[[1]])))] <- 0
  ...
  pr_geo_bpns <- crop(pr_geo_bpns, extent(x_min,x_max,
                                          y_min,y_max))
  
  #save rasters
  for(i in 1:nlayers(pr_geo_bpns)) {
    writeRaster(pr_geo_bpns[[i]], paste0("output_rasters_BPNS/BPNS_",species_sp$simple[s],"_spawning_",i,".tif"))
  }
  
  #save picture
  png(paste0("BPNS_",species_sp$simple[s], "_prediction.png"), width = 8, height = 9, res = 100, units = 'in')
  print(levelplot(pr_geo_bpns, col.regions = colorRampPalette(rev(brewer.pal(10, 'RdYlBu'))), 
                  margin = FALSE, main = paste0("HSI ", species_sp$scientific[s])))
  dev.off()
  print(s)
}
