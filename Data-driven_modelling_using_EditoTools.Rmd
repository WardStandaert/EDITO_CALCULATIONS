...
parameters = list("thetao"= c("par" = "thetao", "fun" = "mean", "buffer" = "10000"),
                  ...
                  "zooc"= c("par" = "zooc", "fun" = "mean", "buffer" = "10000", "convert_from_timestep" = 86400000),
                  "phyc"= c("par" = "phyc", "fun" = "mean", "buffer" = "10000"))
...
save(df_occ_bg_env, file = "zarr_extraction/SAVE/pres_abs_env_save_EditoTools.Rdata")
```
...
```{r step 6, eval = FALSE}
load(file = "zarr_extraction/SAVE/pres_abs_env_save_EditoTools.Rdata")
...
save(model_fit, file = "zarr_extraction/SAVE/model_fit_save_EditoTools.Rdata")
```
...
```{r step 7.2}
load("zarr_extraction/SAVE/model_fit_save_EditoTools.Rdata")
...
for (d in c("2019-09-01", "2019-10-01", "2019-12-01", "2020-01-01")) {
  for (i in 1:length(variables)) {
  raster_list[[d]][[i]] <- getRasterSlice2(variables[i],
                                      stacCatalogue = EDITOSTAC,
                                      lon_min = -12,
                                      lon_max = 10,
                                      lat_min = 48,
...
for (m in unique(df_occ_bg_env$month)) {
  pred_m <- predict(st, mod, clamp=T, type="cloglog")
