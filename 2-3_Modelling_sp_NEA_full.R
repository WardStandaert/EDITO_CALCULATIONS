...
setwd("~/OUTPUT/all_data_model/spawning")
...
# for (s in 1:nrow(species_sp)) write.csv(pr_coord[[s]], paste0("larvae_presences_",species$simple[s],".csv"))
# for (s in 1:nrow(species_sp)) write.csv(bg_coord[[s]], paste0("larvae_background_points_",species$simple[s],".csv"))
...
eval_res_list_sp <- eval_res_list_sp
save(eval_res_list_sp, file = "SAVE/NEA_ENMeval_outcomes.Rdata")
...
#   
#   eval_res_list_sp[[s]] <- ENMevaluate(occs = tmp_pr_predv[[s]], 
#                                     bg = tmp_bg_predv[[s]],
...
# save(eval_res_list_sp, file = "ENMeval_spec_vars_d_no-m_L_LQ_LQH_H_1_5.Rdata")
# 
...
# write.csv(eval.results(eval_res_list_sp[[1]]), "ENMeval_results_herring.csv")
# write.csv(eval.results(eval_res_list_sp[[2]]), "ENMeval_results_mackerel.csv")
# write.csv(eval.results(eval_res_list_sp[[3]]), "ENMeval_results_twaite_shad.csv")
# write.csv(eval.results(eval_res_list_sp[[4]]), "ENMeval_results_seabass.csv")
# 
# for(s in 1:nrow(species_sp)) print(eval_res_list_sp[[s]] %>%  eval.results() %>% filter(delta.AICc == 0))
...
setwd("/data/home/innovauth/ward.standaert@vliz.be/OUTPUT/all_data_model/spawning")

write.csv(sum_all, "NEA_validation_metrics.csv")
...
## boxplots ####
png("NEA_AUC_boxplot.png", width = 6, height =6, res = 100, units = 'in')
...
png("NEA_TSS_boxplot.png", width = 6, height =6, res = 100, units = 'in')
par(las = 2, mar = c(6, 5, 2, 2), mgp = c(3, 0.5, 0))
boxplot(
...
for (s in 1:nrow(species_sp)) {
  ...
  #get model
  eval_res <- eval_res_list_sp[[s]]
  opt.aicc <- eval.results(eval_res_list_sp[[s]]) %>% filter(delta.AICc == 0)
...
#After sp & geo filtering:  Moran's I value of 0.12, expected -0.01; significant
morans_I_list[[2]]   
#Before filtering:          Moran's I value of 0.22, expected -; significant
#After sp & geo filtering:  Moran's I value of 0.11, expected -0.01; significant
...
# x_red <- full_dat_sp %>% filter(scientificname == species_sp$scientific[s])
# 
# tab_fun <- function(l) {
...
sort(cor_maxnet_sp[[1]], decreasing = TRUE)
sort(cor_maxnet_sp[[2]], decreasing = TRUE)
...
for (s in 1:nrow(species_sp)) {
  png(paste0("NEA_",species_sp$simple[s], "_var_imp_response_curves.png"), width = 13, 
  ...
  par(mar=c(5,6,4,2))
  barplot(sort(cor_maxnet_sp[[s]][which(cor_maxnet_sp[[s]] != 0)]), horiz = T, las = 1, xlab = "variable importance")
  
  #response curves
  v_imp <- sort(cor_maxnet_sp[[s]], decreasing = TRUE)
  for (v in 1:length(which(v_imp > 0.03))) {
...
# 
# mod$levels
# plot_st_test <- stack(raster(matrix(c(1,1,1,1),nrow=2)),
#                       raster(matrix(c(1,1,1,1),nrow=2)),
#                       raster(matrix(c(1,1,1,1),nrow=2)),
#                       raster(matrix(c(1,1,1,1),nrow=2)),
#                       raster(matrix(c(1,1,1,1),nrow=2)), #energy
#                       raster(matrix(c(3,2,1,1),nrow=2)), #substrate
...
for (y in 1:length(2000:2020)) {
  for (s in 1:nrow(species_sp)) {
    ...
    #save picture
    png(paste0("output_rasters_NEA/NEA_",c(2000:2020)[y],"_",species_sp$simple[s], "_prediction_NEA.png"), width = 8, height = 9, res = 100, units = 'in')
    ...
    if(y == 21) {
      for(i in 1:nlayers(pr_geo)) {
        writeRaster(pr_geo[[i]], paste0("output_rasters_NEA/NEA_",species_sp$simple[s],"_spawning_",c(2000:2020)[y],"_",i,".tif"), overwrite = T)
      }
    }
    print(paste0(c(2000:2020)[y],"_",species_sp$simple[s]))
  }
}
...
for (m in 1:12) {
  ...
  writeRaster(av_NEA, paste0("output_rasters_NEA/NEA_herring_larvae_average_HSI_",m,".tif"), overwrite = T)
  writeRaster(sd_NEA, paste0("output_rasters_NEA/NEA_herring_larvae_sd_HSI_",m,".tif"), overwrite = T)
  writeRaster(uncert_NEA, paste0("output_rasters_NEA/NEA_herring_larvae_CV_HSI_",m,".tif"), overwrite = T)
  ...
  ggsave(paste0("evolution/larvae_yearly_evolution_HSI_", m,".png"), width = 10, height = 4)
...
for (y in 1:length(2000:2020)) {
  ...
  ggsave(paste0("evolution/larvae_monthly_evolution_HSI_", c(2000:2020)[y],".png"), width = 10, height = 4)
